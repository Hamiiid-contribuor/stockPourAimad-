...
public class ProduitController implements Serializable {
    ...
    public List<Produit> findByAbonne() {
        return ejbFacade.findByAbonne(SessionUtil.getConnectedUser().getAbonne(), 0);
    }
    ...
    public List<Produit> getItems() {
        if (items == null) {
            items = ejbFacade.findByAbonne(SessionUtil.getConnectedUser().getAbonne(), 0);
        }
        return items;
    }
